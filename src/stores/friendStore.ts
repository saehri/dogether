  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        updateFriendStatus: (friendId, isOnline) => set((state) => ({
...
export const useFriendsLoading = () => useFriendStore((state) => state.isLoading);
...
export const useFriendById = (friendId: string) =>
  useFriendStore((state) => state.friends.find(friend => friend.id === friendId));